...
Note: NM (Normalized Mutation) is NOT a variant. It is a property of the SLIM
version itself — SLIM+N1 / SLIM*N1 are the versions that use normalized
mutation. The orthogonal variant flags are OMS, LS (Linear Scaling),
Pareto Tournament, and Simplification.
"""
...
def build_execution_type(use_linear_scaling=False, use_oms=False,
    ...
    """
    Build execution type name based on enabled variant flags.
...
def build_variant_name(slim_version, use_oms=False, use_linear_scaling=False,
    ...
    Args:
        slim_version: SLIM version (e.g., "SLIM+ABS", "SLIM+SIG2", "SLIM+N1")
        use_oms: Whether OMS is enabled
        use_linear_scaling: Whether linear scaling is enabled
        use_pareto_tournament: Whether Pareto tournament is enabled
